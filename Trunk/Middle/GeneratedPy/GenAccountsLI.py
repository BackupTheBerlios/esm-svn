'''
GenAccountsLI.py
Sun Oct 27 19:38:43 2002
Generated by MiddleKit.
'''

# MK attribute caches for setFoo() methods
_IDAttr = None
_LieferantNrAttr = None
_VornameAttr = None
_NachnameAttr = None
_TitelAttr = None
...
class GenAccountsLI(MiddleObject):

	def __init__(self):
		MiddleObject.__init__(self)
		self._ID            = None
		self._LieferantNr   = None
		self._Vorname       = None
		self._Nachname      = None
		self._Titel         = None
	...
	def LieferantNr(self):
		return self._LieferantNr

	def setLieferantNr(self, value):
		assert value is not None
		if value is not None:
			if type(value) is not types.StringType:
				raise TypeError, 'expecting string type, but got value %r of type %r instead' % (value, type(value))
		...
		# set the attribute
		origValue = self._LieferantNr
		self._LieferantNr = value

		# MiddleKit machinery
		self._mk_changed = 1  # @@ original semantics, but I think this should be under "if not self._mk_initing..."
		if not self._mk_initing and self._mk_serialNum>0 and value is not origValue:
			global _LieferantNrAttr
			if _LieferantNrAttr is None:
				_LieferantNrAttr = self.klass().lookupAttr('LieferantNr')
				if not _LieferantNrAttr.shouldRegisterChanges():
					_LieferantNrAttr = 0
			if _LieferantNrAttr:
				# Record that it has been changed
				if self._mk_changedAttrs is None:
					self._mk_changedAttrs = {} # maps name to attribute
				self._mk_changedAttrs['LieferantNr'] = _LieferantNrAttr  # changedAttrs is a set
				# Tell ObjectStore it happened
				self._mk_store.objectChanged(self)
