...
class PersonalChange(SiteTemplate):
  ...
  def writeContent(self, trans=None):
    ...
    if len(storeObjects) == 1:
      ...
      for attr in personalAttrs:
        if (attr not in standardAttrs) and self.transaction.request().field(attr,None) != None:
          ...
          elif personalFields[attr][personalType] == 'datetime':
            if string.strip(self.transaction.request().field(attr,'')):
              personal.setValueForKey(attr, DateTime.DateTimeFrom(self.transaction.request().field(attr,'')))
            else:
              personal.setValueForKey(attr, None)
          elif personalFields[attr][personalType] == 'date':
    ...
    #
    self.transaction.response().sendRedirect('PersonalView?index=' + account[2:])
